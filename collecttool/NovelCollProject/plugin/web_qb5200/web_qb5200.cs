...
namespace NovelCollProject.plugin.web_qb5200
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:qb5200.org/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:qb5200.org/\w+/\d+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:qb5200.org/\w+/\d+/\d+/\d+.html");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//table[@class='a']//td");
            if (linkNodes != null)
            {
