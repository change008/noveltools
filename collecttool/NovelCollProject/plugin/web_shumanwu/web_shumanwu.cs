...
namespace NovelCollProject.plugin.web_shumanwu
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:shumanwu.net/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:shumanwu.net/\d+/\d+/index.html\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:shumanwu.net/\d+/\d+/\d+.html");
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//ul[@class='list-group list-charts']/li");
            if (linkNodes != null)
            {
