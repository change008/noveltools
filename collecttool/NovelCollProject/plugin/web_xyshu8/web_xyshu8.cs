...
namespace NovelCollProject.plugin.web_xyshu8
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:xyshu8.net/\w+/\w+/\d+/\d+/index.html\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:xyshu8.net/\w+/\w+/\d+/\d+/\d+.html");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//div[@id='BookText']/ul/li");
            if (linkNodes != null)
            {
