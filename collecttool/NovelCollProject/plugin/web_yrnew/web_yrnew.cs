...
namespace NovelCollProject.plugin.web_yrnew
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:xxxxxx/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:yrnew.net/plugin.php\?id=jameson_read&contrl=index&act=book&book_id=\d+");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:yrnew.net/plugin.php\?id=jameson_read&contrl=index&act=read&tid=\d+&pid=\d+");
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//div[@class=\"zhangjie pure-g\"]/a");
            if (linkNodes != null)
            {
