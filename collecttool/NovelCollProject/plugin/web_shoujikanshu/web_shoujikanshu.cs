...
namespace NovelCollProject.plugin.web_shoujikanshu
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:shoujikanshu.org/\w+/\w+_\d+.html\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:shoujikanshu.org/\w+/\d+(_\d+)?.html");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Match m;
            Match m1;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//div[@class='list']/li");
            if (linkNodes != null)
            {
                ...
                foreach (var liNode in linkNodes)
                {
                    ...
                    reg = new Regex(@"\d+_(\d+).html$");
                    if (reg.IsMatch(url))
                    {
                        m = reg.Match(url);
                        flag = reg.Replace(m.Value, "$1");
                    }
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            if (tempNode != null)
            {
                ...
                tempString = tempString.Replace("\r\n", "").Replace("\t", "")
                    .Replace("<p class=\"addresses\">文章地址：<a href=\"www.shoujikanshu.org/xiaoshuo/18594.html\">www.shoujikanshu.org/xiaoshuo/18594.html</a></p>","")
                    .Replace("www.shoujikanshu.org","")
                    .Replace("免费小说", "")
