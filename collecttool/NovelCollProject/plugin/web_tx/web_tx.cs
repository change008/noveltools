...
namespace NovelCollProject.plugin.web_tx
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:tiexue.net/\w+/\w+/ContentList.aspx\?ChapterID=\d+");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:tiexue.net/\w+/ContentEdit.aspx\?ContentID=\d+");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//table[@id='DataGrid1']/tr");
            if (linkNodes != null)
            {
                links = new List<Hashtable>();
                foreach (var liNode in linkNodes)
                {
                    var title = liNode.ChildNodes[2]?.FirstChild?.FirstChild?.InnerText;
                    var url = liNode.ChildNodes[2]?.FirstChild?.FirstChild?.GetAttributeValue("href", "");
                    if (string.IsNullOrEmpty(url))
                        continue;
                    reg = new Regex(@"ContentID=(\d+)");
                    m = reg.Match(url);
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            Regex tempReg = null;
            Match tempMatch = null;
            tempNode = documentNode.SelectSingleNode("//textarea[@id='BodyBox']");
            if (tempNode != null)
            {
