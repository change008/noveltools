...
namespace NovelCollProject.plugin.web_xqqxs
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:xqqxs.com/\w+/\d+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:xqqxs.com/\w+/\d+/\d+/\d+.html");
        ...
        private Hashtable parseStartupPage(HtmlNode documentNode)
        {
            ...
            HtmlNode linkNodes = documentNode.SelectSingleNode("//div[@id='list']/dl/dd");
            if (linkNodes != null)
            {
