...
namespace NovelCollProject.plugin.web_wxguan
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:wenxueguan.com/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:wxguan.com/\w+/\d+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:wxguan.com/\w+/\d+/\d+/\d+.html");
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Match m;
            int i = 0;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//div[@class='listmain']/dl/dd");
            if (linkNodes != null)
            {
