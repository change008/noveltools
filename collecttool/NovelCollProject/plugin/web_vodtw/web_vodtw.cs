...
namespace NovelCollProject.plugin.web_vodtw
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:vodtw.com/\w+/\w+/\d+/\d+/index.html\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:vodtw.com/\w+/\w+/\d+/\d+/\d+.html");
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//div[@class='insert_list']/dl/dd/ul/li");
            if (linkNodes != null)
            {
