...
namespace NovelCollProject.plugin.web_xiangcunxiaoshuo
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:kanshula.com/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:xiangcunxiaoshuo.com/\w+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:xiangcunxiaoshuo.com/\w+/\d+/\d+.html");
        }
        ...
        public override void Load(bool isUTF8, int bookId = 0)
        {
            if (PageFeature.MatchURL(Url) == PageTypeEnum.ListPage1)
            {
                ...
                SetCookie("UM_distinctid=15db7551c54a11-0bd078b9df4594-59462f1d-1fa400-15db7551c55bd4; bdshare_firstime=1502019404852; CNZZDATA5597879=cnzz_eid%3D1315950026-1502018034-null%26ntime%3D1504183434; BLR=410377%2337701334%23%u7231%u4E0A%u591C%u73AB%u7470%23%20%u7B2C%u4E00%u7AE0%uFF1A%u6211%u7684%u7238%u7238%uFF01");
                SetHost("www.xiangcunxiaoshuo.com");
            }
            else
            {
                SetUserAgent("Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.101 Safari/537.36");
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//section[@class='ml_main']/dl/dd");
            if (linkNodes != null)
            {
