...
namespace NovelCollProject.plugin.web_yilego
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:yilego.com/\w+/\d+/\d+/index.html");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:yilego.com/\w+/\d+/\d+/\d+.html");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//ul[@class='float-list fill-block']/li");
            if (linkNodes != null)
            {
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            Regex tempReg = null;
            Match tempMatch = null;
            tempNode = documentNode.SelectSingleNode("//div[@id='ChapterContent dssds']");
            if (tempNode != null)
            {
