...
namespace NovelCollProject.plugin.web_sjtxt
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:sjtxt.la/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:sjtxt.la/\w+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:sjtxt.la/\w+/\d+/\d+.html");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes1 = documentNode.SelectNodes("//div[@class='pc_list']");
            HtmlNodeCollection linkNodes = linkNodes1[1].ChildNodes[3].SelectNodes("li");
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            Regex tempReg = null;
            Match tempMatch = null;
            tempNode = documentNode.SelectSingleNode("//div[@id='content1']");
            if (tempNode != null)
            {
