...
namespace NovelCollProject.plugin.web_2wxs
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:2wxs.com/\w+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:2wxs.com/\w+/\d+/\d+.html");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            if (linkNodes != null)
            {
                ...
                foreach (var liNode in linkNodes)
                {
                    if (ifPre)
                    {
                        var titleTemp = liNode.SelectSingleNode("a")?.InnerText;
                        if (titleTemp == "第 1 章 初出茅庐")
                        {
                            ifPre = false;
                        }
                        else
