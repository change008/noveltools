...
namespace NovelCollProject.plugin.web_sangwu
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:sangwu.org/\w+/\d+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:sangwu.org/\w+/\d+/\d+/\d+.htm");
        }
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg;
            Match m;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//div[@class='main']/dl/dd");
            if (linkNodes != null)
            {
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            Regex tempReg = null;
            Match tempMatch = null;
            tempNode = documentNode.SelectSingleNode("//div[@class='centent']");
            if (tempNode != null)
            {
