...
namespace NovelCollProject.plugin.web_d9cn
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:d9cn.net/\w+/\d+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:d9cn.net/\w+/\d+/\d+/\d+.html");
        }
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            if (tempNode != null)
            {
                ...
                tempString = tempString.Replace("\r\n", "").Replace("\t", "")
                    ...
                    .Replace("WWw.ZiyoUgE.com", "")
                    .Replace("[【第&amp;九^中文^D9cn.net$】", "")
                    .Replace("【第九zhong文*Www^D9cn.Net*为你更新更多精彩内容】", "")

                    ;

                //正则替换域名
                string patterna = @"【.*?】";
                tempString = Regex.Replace(tempString, patterna, "");
