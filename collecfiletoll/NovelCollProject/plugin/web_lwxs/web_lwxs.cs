...
namespace NovelCollProject.plugin.web_lwxs
{
    class Page : PageBase
    {
        ...
        {
            pageFeature.InjectUrlRule(PageTypeEnum.StartupPage, @"REG:lwxs.com/nonono$");
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:lwxs.com/\w+/\d+/\d+/\?chapterlist");
            pageFeature.InjectUrlRule(PageTypeEnum.DetailPage1, @"REG:lwxs.com/\w+/\d+/\d+/\d+.html");
        ...
        private Hashtable parseDetailPage1(HtmlNode documentNode)
        {
            ...
            Regex tempReg = null;
            Match tempMatch = null;
            tempNode = documentNode.SelectSingleNode("//div[@id=\"TXT\"]");
            if (tempNode != null)
            {
