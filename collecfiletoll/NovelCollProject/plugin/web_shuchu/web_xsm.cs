...
namespace NovelCollProject.plugin.web_shuchu
{
    class Page : PageBase
    {
        ...
        protected override void buildPageFeatureRules(PageFeature pageFeature)
        {
            ...
            pageFeature.InjectUrlRule(PageTypeEnum.ListPage1, @"REG:/book/chapterlist-\d+.html");
        ...
        private Hashtable parseListPage(HtmlNode documentNode)
        {
            ...
            Regex reg  ;
            Match m ;
            HtmlNodeCollection linkNodes = documentNode.SelectNodes("//ul[@class='chapter_list']/li");
            if (linkNodes != null)
            {
                links = new List<Hashtable>();
                foreach (var liNode in linkNodes)
                {
                    var title = liNode.SelectSingleNode("a/span[1]").InnerText;
            ...
            HtmlNode nextLink = documentNode.SelectNodes("//div[@class='page_bar clear']/ul/li/a").FirstOrDefault(x => x.InnerText == "下一页");
            if (nextLink != null&& links!=null&& links.Count>1)
            {

                int bookId = 0;
                reg = new Regex(@"book/chapterlist-(\d+)");
                ...
                int p = 1;
                reg = new Regex(@"page=(\d+)");
                m = reg.Match(nextLink.GetAttributeValue("href",""));
                if (m.Success)
                {
                    p = Convert.ToInt32(reg.Replace(m.Value, "$1"));
                }
                else
                    p++;
                var nexturl =string.Format("http://m.shuchu.com/book/chapterlist-{0}.html?page={1}", bookId,p);
