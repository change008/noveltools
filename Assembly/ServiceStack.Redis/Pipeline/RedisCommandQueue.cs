...
namespace ServiceStack.Redis
{
    ...
    public  class RedisCommandQueue : RedisQueueCompletableOperation
   {
        ...
        public void QueueCommandVoidReadCommand(Action<IRedisClient> command ,Action voidReadCommand)
        {
            QueueCommand(command, null, null, voidReadCommand);
        }
        ...
        /// <param name="voidReadCommand"></param>
        public virtual void QueueCommand(Action<IRedisClient> command, Action onSuccessCallback, Action<Exception> onErrorCallback,Action voidReadCommand)
        {
            BeginQueuedCommand(new QueuedRedisCommand
            {
                VoidReturnCommand = command,
                OnSuccessVoidCallback = onSuccessCallback,
                OnErrorCallback = onErrorCallback,
                VoidReadCommand = voidReadCommand
            });
            command(RedisClient);
        }
